...
const MobilePreview: React.FC<MobilePreviewProps> = ({ 
  ...
  runtimeError, onAutoFix, isEditorMode
}) => {
  ...
  return (
    <section className={`flex-1 flex flex-col items-center ${workspace === 'admin' ? 'lg:items-stretch lg:px-6' : (isEditorMode ? 'lg:items-center' : 'lg:items-start lg:pl-40')} lg:justify-center relative h-full transition-all duration-1000 ${mobileTab === 'chat' && !isEditorMode ? 'hidden lg:flex' : 'flex'} ${isEditorMode ? 'w-full' : ''}`}>
      ...
      <div className={`w-full h-full lg:h-auto lg:flex-1 flex flex-col ${workspace === 'admin' ? 'lg:items-stretch' : 'lg:items-center'} justify-center p-0 pt-20 pb-20 lg:p-4`}>
          <div className={`relative group/preview-container transition-all duration-700 hover:scale-[1.01] w-full h-full ${isEditorMode ? 'scale-[0.85] origin-top' : ''}`}>
