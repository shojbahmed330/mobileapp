    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(`OpenRouter error: ${response.status} ${response.statusText} - ${JSON.stringify(errorData)}`);
        }
        ...
        Logger.warn(`Attempt ${attempt} failed for OpenRouter`, { component: 'GeminiService', model, attempt }, error);
        lastError = error;
        if (attempt < retries) {
